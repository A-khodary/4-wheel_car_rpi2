...
int GPIO::setDirection(GPIO_DIRECTION dir){
   switch(dir){
   case GINPUT: return this->write(this->path, "direction", "in");
      break;
   case GOUTPUT:return this->write(this->path, "direction", "out");
      break;
   }
   return -1;
}
...
int GPIO::setValue(GPIO_VALUE value){
   switch(value){
   case GHIGH: return this->write(this->path, "value", "1");
      break;
   case GLOW: return this->write(this->path, "value", "0");
      break;
   }
   return -1;
}
...
int GPIO::toggleOutput(){
	this->setDirection(GOUTPUT);
	if ((bool) this->getValue()) this->setValue(GLOW);
	else this->setValue(GHIGH);
    return 0;
}
...
void* threadedToggle(void *value){
	...
	while(gpio->threadRunning){
		if (isHigh)	gpio->setValue(GHIGH);
		else gpio->setValue(GLOW);
...
int GPIO::waitForEdge(){
	this->setDirection(GINPUT); // must be an input pin to poll its value
